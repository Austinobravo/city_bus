...
function AccordionTrigger({
  ...
  return (
    <AccordionPrimitive.Header className="flex">
      ...
      >
        {children}
        <Plus className="text-muted-foreground group-data-[state=open]:hidden pointer-events-none size-4 shrink-0 translate-y-0.5 transition-transform duration-200" />
        <Minus className="text-muted-foreground group-data-[state=open]:block hidden pointer-events-none size-4 shrink-0 translate-y-0.5 transition-transform duration-200" />
      </AccordionPrimitive.Trigger>
